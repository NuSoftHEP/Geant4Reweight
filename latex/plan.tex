...
\end{equation}
The entire weight for a track that undergoes an interaction is then\footnote {$\sigma_{a,s}'$ in the numerator of the first term accounts for the case that the cross section for process $a$ is varied. It could be held the same in the case the process which occurred is not varied.}:
...
In addition to that 'elastic-survival' factor, multiple weights are assigned from the start of the track to the first elastic scatter and then between any subsequent elastic scatters. These take the form of equation \ref{eq:10}. We ignore the $\frac{R'}{R}$ factor used for the other discrete processes, as track-preserving factors can be treated as essentially independent of one another. 

The resulting weight then takes the form of:
\begin{equation}
W_{elast} = (\prod \limits_{elast} \frac{1 - e^{-\sum L_s  \sigma_{s}'}}{1 - e^{-\sum L_s  \sigma_{s}}})(\frac{e^{-\sum L_s \sigma_{s}'}}{e^{-\sum L_s \sigma_{s}}})
\end{equation}
%\begin{figure}
%\includegraphics[width=.8\textwidth]{full_comparison.pdf}
%\includegraphics[width=.8\textwidth]{zoomed_comparison.pdf}
%\end{figure}
%
%
%
%\begin{figure}
%\includegraphics[width=\textwidth]{decay_w_len.pdf}
%\includegraphics[width=\textwidth]{decay_w_p.pdf}
%\end{figure}


%By inspecting equations \ref{eq:1}, \ref{eq:5},  and \ref{eq:6}
