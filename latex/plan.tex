...
P(N) = e^{-N}
\end{equation}
at the beginning of a single track and after each time that process is invoked\footnote{Discrete processes can be furthered categorized depending on if they kill a track (such as pion-inelastic scattering) or the track is still alive after the process is invoked (hadron-elastic scattering). This will affect the treatment of weights slightly, and will be discussed further.\label{fn_Discrete}}. This number, $N_p$ for a given Discrete process, is used indirectly in determining the proposed interaction length for that process each step. In a specific step, each active Discrete process has an interaction length $L_p$ which is determined by the mean free path of that process as such: 
\begin{equation}
L_p = N_p * \lambda_p
\end{equation}
Each Continuous process also proposes an interaction length. The smallest option out of the Discrete and Continous processes and of the distance to the next boundary is chosen. If a Discrete process was chosen (and if the track is still alive), that process has another $N_p$ sampled from \ref{eq:sample}. Each Discrete process which was not chosen to occur has its own previously-sampled $N_p$ reduced by an amount equal to $\frac{L_s}{\lambda_p}$, where $L_s$ is the step length which was chosen in that step. These new $N_p$ are then used in the next step in determining the interaction lengths.

%This number, $N$, is essentially the number of interaction lengths - for a given process - through which the particle will travel, after which that process will be invoked. It is instructive to view N as such: 
%\begin{equation}\label{eq:3}
%N = \sum\limits_{s={steps}} \frac{L_{s}}{\lambda_{s}}
%\end{equation}
%where $N$ is the sampled number for a given process, $\lambda_s$ is the mean free path of that same process in any specific step, and $L_s$ is the length traveled for that specific step. An important point is that each specific $L_s$ is the same for all processes active in that step. 

In practice, this results in a static \textit{number of interaction lengths} for a given process, where those interaction lengths are constantly updated (changes of kinematics or target).
...
Figures \ref{ref:thin_1.5_1} through \ref{ref:thin_1.5_1.5} compare the nominal cross sections to the varied and weighted samples from Table \ref{ref:variations}.

\begin{figure}[htpb]\label{ref:thin_1.5_1}
	\centering
...
\begin{figure}[htpb]\label{ref:thin_1_1.5}
	\centering
	\begin{subfigure}[t!]{.45\textwidth}
		\centering
...
\begin{figure}[htpb]\label{ref:thin_1.5_1.5}
	\centering
	\begin{subfigure}[t!]{.45\textwidth}
		\centering
...
The results of this are shown in Figures asdfadsf through asdfasdf
...
%	\begin{subfigure}[t!]{.45\textwidth}
%		\centering
%		\includegraphics[width=\textwidth]{{/home/jake/comp_pdfs/reactive_slice_xsec_inel1.5_elast1.5}.pdf}
%	\end{subfigure}
%	~
	\begin{subfigure}[t!]{.45\textwidth}
		\centering
