...
#include "G4BGGPionElasticXS_bias.hh"
...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronElasticPhysics_bias);
//
G4ThreadLocal G4bool G4HadronElasticPhysics_bias::wasActivated = false;
G4ThreadLocal G4HadronElastic* G4HadronElasticPhysics_bias::neutronModel = 0;
G4ThreadLocal G4HadronicProcess* G4HadronElasticPhysics_bias::neutronProcess = 0;

G4HadronElasticPhysics_bias::G4HadronElasticPhysics_bias(G4int ver, G4double bias)
  : G4VPhysicsConstructor("hElasticWEL_CHIPS"), verbose(ver), theBias(bias)
{
  if(verbose > 1) { 
    G4cout << "### G4HadronElasticPhysics_bias: " << GetPhysicsName() 
	   << G4endl; 
  }
}

G4HadronElasticPhysics_bias::~G4HadronElasticPhysics_bias()
{}

void G4HadronElasticPhysics_bias::ConstructParticle()
{
  // G4cout << "G4HadronElasticPhysics_bias::ConstructParticle" << G4endl;
...
void G4HadronElasticPhysics_bias::ConstructProcess()
{
  ...
  if(verbose > 1) {
    G4cout << "### HadronElasticPhysics_bias::ConstructProcess: Elimit for pi " 
  ...
  while( (*myParticleIterator)() )
  {
    ...
    if(pname == "anti_lambda"  ||
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_bias: " << hel->GetProcessName()
	...
	G4cout << "### HadronElasticPhysics_bias: " << hel->GetProcessName()
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_bias: " 
      ...
      G4HadronElasticProcess* hel = new G4HadronElasticProcess();
      hel->AddDataSet(new G4BGGPionElasticXS_bias(particle, theBias));
...
G4HadronElastic* G4HadronElasticPhysics_bias::GetNeutronModel()
{
  return neutronModel;
}

G4HadronicProcess* G4HadronElasticPhysics_bias::GetNeutronProcess()
{
