...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronElasticPhysics_binned);
//
G4ThreadLocal G4bool G4HadronElasticPhysics_binned::wasActivated = false;
G4ThreadLocal G4HadronElastic* G4HadronElasticPhysics_binned::neutronModel = 0;
G4ThreadLocal G4HadronicProcess* G4HadronElasticPhysics_binned::neutronProcess = 0;

G4HadronElasticPhysics_binned::G4HadronElasticPhysics_binned(G4int ver, G4ReweightHist * bias_hist)
  : G4VPhysicsConstructor("hElasticWEL_CHIPS"), verbose(ver), theBias(bias_hist)
{
  if(verbose > 1) { 
    G4cout << "### G4HadronElasticPhysics_binned: " << GetPhysicsName() 
	   << G4endl; 
  }
}

G4HadronElasticPhysics_binned::~G4HadronElasticPhysics_binned()
{}

void G4HadronElasticPhysics_binned::ConstructParticle()
{
  // G4cout << "G4HadronElasticPhysics_binned::ConstructParticle" << G4endl;
...
void G4HadronElasticPhysics_binned::ConstructProcess()
{
  ...
  if(verbose > 1) {
    G4cout << "### HadronElasticPhysics_binned::ConstructProcess: Elimit for pi " 
  ...
  while( (*myParticleIterator)() )
  {
    ...
    if(pname == "anti_lambda"  ||
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_binned: " << hel->GetProcessName()
	...
	G4cout << "### HadronElasticPhysics_binned: " << hel->GetProcessName()
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_binned: " 
      ...
      G4HadronElasticProcess* hel = new G4HadronElasticProcess();
      hel->AddDataSet(new G4BGGPionElasticXS_binned(particle, theBias));
...
G4HadronElastic* G4HadronElasticPhysics_binned::GetNeutronModel()
{
  return neutronModel;
}

G4HadronicProcess* G4HadronElasticPhysics_binned::GetNeutronProcess()
{
