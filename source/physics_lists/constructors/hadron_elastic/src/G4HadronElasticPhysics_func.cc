...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronElasticPhysics_func);
//
G4ThreadLocal G4bool G4HadronElasticPhysics_func::wasActivated = false;
...
void G4HadronElasticPhysics_func::ConstructParticle()
{
  // G4cout << "G4HadronElasticPhysics_func::ConstructParticle" << G4endl;
...
void G4HadronElasticPhysics_func::ConstructProcess()
{
  ...
  if(verbose > 1) {
    G4cout << "### HadronElasticPhysics_func::ConstructProcess: Elimit for pi " 
  ...
  while( (*myParticleIterator)() )
  {
    ...
    if(pname == "anti_lambda"  ||
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_func: " << hel->GetProcessName()
	...
	G4cout << "### HadronElasticPhysics_func: " << hel->GetProcessName()
      ...
      if(verbose > 1) {
	G4cout << "### HadronElasticPhysics_func: " 
      ...
      G4HadronElasticProcess* hel = new G4HadronElasticProcess();
      hel->AddDataSet(new G4BGGPionElasticXS_func(particle, theBias));
...
G4HadronElastic* G4HadronElasticPhysics_func::GetNeutronModel()
{
  return neutronModel;
}

G4HadronicProcess* G4HadronElasticPhysics_func::GetNeutronProcess()
{
