...
// ********************************************************************
//
// $Id: G4HadronPhysicsFTFP_BERT_bias.cc 102624 2017-02-10 08:22:14Z gcosmo $
//
//---------------------------------------------------------------------------
//
// ClassName:   
...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronPhysicsFTFP_BERT_bias);

G4ThreadLocal G4HadronPhysicsFTFP_BERT_bias::ThreadPrivate* G4HadronPhysicsFTFP_BERT_bias::tpdata=0;

G4HadronPhysicsFTFP_BERT_bias::G4HadronPhysicsFTFP_BERT_bias(G4int, G4double bias)
    :  G4VPhysicsConstructor("hInelastic FTFP_BERT_bias"), theBias(bias)
/*    , theNeutrons(0)
    , theBertiniNeutron(0)
    , theFTFPNeutron(0)
    , thePion(0)
...
G4HadronPhysicsFTFP_BERT_bias::G4HadronPhysicsFTFP_BERT_bias(const G4String& name, G4bool quasiElastic, G4double bias)
...
void G4HadronPhysicsFTFP_BERT_bias::CreateModels()
{
  ...
  tpdata->theFTFPPion=new G4FTFPPionBuilder_bias(QuasiElastic,theBias);
  tpdata->thePion->RegisterMe(tpdata->theFTFPPion);
  tpdata->theFTFPPion->SetMinEnergy(minFTFP_pion);
  tpdata->thePion->RegisterMe(tpdata->theBertiniPion=new G4BertiniPionBuilder_bias(theBias));
...
G4HadronPhysicsFTFP_BERT_bias::~G4HadronPhysicsFTFP_BERT_bias()
{
  if (!tpdata) return;
...
void G4HadronPhysicsFTFP_BERT_bias::ConstructParticle()
{
...
void G4HadronPhysicsFTFP_BERT_bias::ConstructProcess()
{
