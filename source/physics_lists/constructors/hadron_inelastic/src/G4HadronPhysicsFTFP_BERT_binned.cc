...
// ********************************************************************
//
// $Id: G4HadronPhysicsFTFP_BERT_binned.cc 102624 2017-02-10 08:22:14Z gcosmo $
//
//---------------------------------------------------------------------------
//
// ClassName:   
...
//
G4_DECLARE_PHYSCONSTR_FACTORY(G4HadronPhysicsFTFP_BERT_binned);

G4ThreadLocal G4HadronPhysicsFTFP_BERT_binned::ThreadPrivate* G4HadronPhysicsFTFP_BERT_binned::tpdata=0;

G4HadronPhysicsFTFP_BERT_binned::G4HadronPhysicsFTFP_BERT_binned(G4int, G4ReweightHist * bias_hist)
    :  G4VPhysicsConstructor("hInelastic FTFP_BERT_binned"), theBias(bias_hist)
/*    , theNeutrons(0)
    , theBertiniNeutron(0)
    , theFTFPNeutron(0)
    , thePion(0)
...
G4HadronPhysicsFTFP_BERT_binned::G4HadronPhysicsFTFP_BERT_binned(const G4String& name, G4bool quasiElastic, G4ReweightHist * bias_hist)
...
void G4HadronPhysicsFTFP_BERT_binned::CreateModels()
{
  ...
  tpdata->theFTFPPion=new G4FTFPPionBuilder_binned(QuasiElastic,theBias);
  tpdata->thePion->RegisterMe(tpdata->theFTFPPion);
  tpdata->theFTFPPion->SetMinEnergy(minFTFP_pion);
  tpdata->thePion->RegisterMe(tpdata->theBertiniPion=new G4BertiniPionBuilder_binned(theBias));
...
G4HadronPhysicsFTFP_BERT_binned::~G4HadronPhysicsFTFP_BERT_binned()
{
  if (!tpdata) return;
...
void G4HadronPhysicsFTFP_BERT_binned::ConstructParticle()
{
...
void G4HadronPhysicsFTFP_BERT_binned::ConstructProcess()
{
