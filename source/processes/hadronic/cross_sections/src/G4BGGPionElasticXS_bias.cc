...
G4BGGPionElasticXS_bias::G4BGGPionElasticXS_bias(const G4ParticleDefinition*, G4double bias) 
...
G4BGGPionElasticXS_bias::~G4BGGPionElasticXS_bias()
{
  delete fSAID;
  delete fHadron;
  delete fPion;
...
G4bool 
G4BGGPionElasticXS_bias::IsElementApplicable(const G4DynamicParticle*, G4int,
					   const G4Material*)
{
  return true;
}
...
G4bool G4BGGPionElasticXS_bias::IsIsoApplicable(const G4DynamicParticle*, 
					      G4int Z, G4int A,  
					      const G4Element*,
					      const G4Material*)
{
...
G4double
G4BGGPionElasticXS_bias::GetElementCrossSection(const G4DynamicParticle* dp,
					   G4int ZZ, const G4Material*)
{
  ...
  if(verboseLevel > 1) {
    G4cout << "G4BGGPionElasticXS_bias::GetElementCrossSection  for "
  ...
  return theBias*cross;
}

G4double
G4BGGPionElasticXS_bias::GetIsoCrossSection(const G4DynamicParticle* dp, 
				       G4int Z, G4int A, 
				       const G4Isotope*,
				       const G4Element*,
				       const G4Material*)
{
  ...
  */
  if(verboseLevel > 1) {
    G4cout << "G4BGGPionElasticXS_bias::GetIsoCrossSection  for "
	   << dp->GetDefinition()->GetParticleName()
	   << "  Ekin(GeV)= " << dp->GetKineticEnergy()
	   << " in nucleus Z= " << Z << "  A= " << A
	   << " XS(b)= " << cross/barn 
...
void G4BGGPionElasticXS_bias::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  if(&p == G4PionPlus::PionPlus() || &p == G4PionMinus::PionMinus()) {
    ...
    G4cout << "### G4BGGPionElasticXS_bias WARNING: is not applicable to " 
	   << p.GetParticleName()
	   << G4endl;
    throw G4HadronicException(__FILE__, __LINE__,
	  "G4BGGPionElasticXS_bias::BuildPhysicsTable is used for wrong particle");
    return;
  }
  ...
  if(verboseLevel > 0) {
    G4cout << "### G4BGGPionElasticXS_bias::Initialise for "
...
void
G4BGGPionElasticXS_bias::CrossSectionDescription(std::ostream& outFile) const 
{
