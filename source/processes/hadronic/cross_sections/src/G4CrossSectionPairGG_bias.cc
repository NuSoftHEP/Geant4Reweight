...
// ********************************************************************
//
// $Id: G4CrossSectionPairGG_bias.cc 94961 2016-01-08 16:31:48Z gcosmo $
// $ GEANT4 tag $Name: not supported by cvs2svn $
//
//   Class G4CrossSectionPairGG_bias
//
...
G4CrossSectionPairGG_bias::G4CrossSectionPairGG_bias(G4PiNuclearCrossSection_bias* low,
      G4double Etransit) :
      G4VCrossSectionDataSet("G4CrossSectionPairGG_bias"), theLowX(low), ETransition(
            Etransit) {
...
G4CrossSectionPairGG_bias::~G4CrossSectionPairGG_bias() {
   delete theHighX;
   // The cross section registry will delete theLowX
}

void G4CrossSectionPairGG_bias::SetBias(double bias){
  theLowX->SetBias(bias);
}

void G4CrossSectionPairGG_bias::CrossSectionDescription(
      std::ostream& outFile) const {
   outFile << "G4CrossSectionPairGG_bias is used to add the relativistic rise to\n"
...
G4bool G4CrossSectionPairGG_bias::IsElementApplicable(
...
G4double G4CrossSectionPairGG_bias::GetElementCrossSection(
      const G4DynamicParticle* aParticle, G4int ZZ, const G4Material* mat)
{
//   G4cout << "GG_bias XSEC" << G4endl;
   G4double Xsec(0.);
...
void G4CrossSectionPairGG_bias::BuildPhysicsTable(const G4ParticleDefinition& pDef) {
G4cout << "BGG_bias TBALLEEEEE" << G4endl; 
   theLowX->BuildPhysicsTable(pDef);
   theHighX->BuildPhysicsTable(pDef);

   if (verboseLevel > 0) {
      G4cout << "G4CrossSectionPairGG_bias::BuildPhysicsTable "
   ...
   if (iter == scale_factors.end()) {
      ...
      if (verboseLevel > 0) {
         G4cout << "G4CrossSectionPairGG_bias::BuildPhysicsTable for particle "
...
void G4CrossSectionPairGG_bias::DumpPhysicsTable(const G4ParticleDefinition&) {
   G4cout << std::setw(24) << " " << " G4CrossSectionPairGG_bias: "
