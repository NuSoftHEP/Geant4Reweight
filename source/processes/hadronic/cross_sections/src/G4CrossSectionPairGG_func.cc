...
// ********************************************************************
//
// $Id: G4CrossSectionPairGG_func.cc 94961 2016-01-08 16:31:48Z gcosmo $
// $ GEANT4 tag $Name: not supported by cvs2svn $
//
//   Class G4CrossSectionPairGG_func
//
...
G4CrossSectionPairGG_func::G4CrossSectionPairGG_func(G4PiNuclearCrossSection_func* low,
      G4double Etransit) :
      G4VCrossSectionDataSet("G4CrossSectionPairGG_func"), theLowX(low), ETransition(
            Etransit) {
...
G4CrossSectionPairGG_func::~G4CrossSectionPairGG_func() {
   delete theHighX;
...
void G4CrossSectionPairGG_func::CrossSectionDescription(
   ...
   outFile << "G4CrossSectionPairGG_func is used to add the relativistic rise to\n"
...
G4bool G4CrossSectionPairGG_func::IsElementApplicable(
...
G4double G4CrossSectionPairGG_func::GetElementCrossSection(
      const G4DynamicParticle* aParticle, G4int ZZ, const G4Material* mat)
{
//   G4cout << "GG_func XSEC" << G4endl;
   G4double Xsec(0.);
...
void G4CrossSectionPairGG_func::BuildPhysicsTable(const G4ParticleDefinition& pDef) {
G4cout << "BGG_func TBALLEEEEE" << G4endl; 
   theLowX->BuildPhysicsTable(pDef);
   theHighX->BuildPhysicsTable(pDef);

   if (verboseLevel > 0) {
      G4cout << "G4CrossSectionPairGG_func::BuildPhysicsTable "
   ...
   if (iter == scale_factors.end()) {
      ...
      if (verboseLevel > 0) {
         G4cout << "G4CrossSectionPairGG_func::BuildPhysicsTable for particle "
...
void G4CrossSectionPairGG_func::DumpPhysicsTable(const G4ParticleDefinition&) {
